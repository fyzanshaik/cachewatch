...
func runCalibrationTests(_ t: TestKit) {
    let base = Date(timeIntervalSince1970: 1_785_000_000)
    let window1 = base.addingTimeInterval(3600)
    let window2 = base.addingTimeInterval(3600 + 18_000)

    t.run("turnCostWeighsTokenTypes") { t in
        ...
        t.expect(abs((cost ?? 0) - 0.1755) < 0.0001, "weighted cost, got \(String(describing: cost))")
        t.expect(Pricing.turnCostUSD(model: "unknown-model", usage: usage) == nil, "unknown model")
    }

    t.run("calibratorFitsDollarsPerPercent") { t in
        var cal = QuotaCalibrator()
        t.expect(cal.dollarsPerPercent == nil, "starts unknown")
        ...
        t.expect(cal.dollarsPerPercent.map { abs($0 - 2.0) < 0.001 } == true, "fits $2/percent, got \(String(describing: cal.dollarsPerPercent))")
        t.expectEqual(cal.percentOfWindow(forCost: 8.0).map { Int($0.rounded()) }, 4, "converts cost to percent")
    }

    t.run("calibratorIgnoresWindowBoundariesAndNoise") { t in
        var cal = QuotaCalibrator()
        ...
        t.expect(cal.dollarsPerPercent == nil, "cross-window pair rejected")
        // Tiny deltas below noise floor are skipped.
        cal.observe(cumulativeCostUSD: 12.01, usedPercentage: 1.1, resetsAt: window2)
        t.expect(cal.dollarsPerPercent == nil, "sub-threshold delta rejected")
        ...
        t.expect(cal.dollarsPerPercent != nil, "fits within new window")
    }

    t.run("calibratorPersistsThroughAppState") { t in
        var cal = QuotaCalibrator()
        ...
        t.expectEqual(restored.calibration?.dollarsPerPercent, cal.dollarsPerPercent, "estimate survives round trip")
    }

    t.run("reducerAccumulatesFleetCost") { t in
        ...
        t.expect(abs(reducer.snapshot.cumulativeTurnCostUSD - 1.5) < 0.0001, "fleet cost accumulates, got \(reducer.snapshot.cumulativeTurnCostUSD)")
    }
}
