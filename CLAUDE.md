...
macOS menu bar app observing all local Claude Code sessions: fleet status, prompt-cache TTL state, quota, memory, alerts. Swift 6 (strict concurrency), macOS 15+, SwiftPM only.

## Commands

- `swift build` — build everything.
- `swift run cachewatch-tests` — THE test suite. Plain-assertion runner (`Sources/CachewatchTests`), exists because this machine has no Xcode (Command Line Tools ship neither XCTest nor Swift Testing). `Tests/CollectorEngineTests/` holds a stale XCTest copy awaiting the Swift Testing port once Xcode is installed; don't extend it, extend the runner.
...
- `Sources/Cachewatch` — thin SwiftUI shell (`MenuBarExtra` `.window` style). Renders snapshots, computes nothing except presentation. Time-derived display (countdowns, warm/cold) is a pure function of snapshot + `now` via `TimelineView`.
