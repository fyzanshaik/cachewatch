...
<p align="center">A macOS menu bar app that watches all your Claude Code and Codex sessions at once: live status, context, quota, per-session memory, and Claude prompt-cache TTL.</p>
...
- **Fleet view.** Every live Claude Code and Codex session: agent, project, branch, model, context fill bar, status, memory of its full process tree including MCP servers, and which terminal app hosts it (click to focus). Sessions waiting on you sort first and show a count in the menu bar icon.
...
- **Quota.** Separate Claude and Codex bars for the windows each agent reports, with reset countdowns. Values merge monotonically per window so stale data from idle sessions can never make quota go backwards. Claude quota survives restarts, and every Claude sample is logged to a 30-day history file for burn-rate analysis.
...
- **Close session.** Hover a row for a confirm-gated button that kills that Claude Code or Codex process.
...
Codex discovery uses `lsof` to join only rollout files held open by live Codex processes, so historical sessions never appear in the fleet. One reducer turns all sources into an immutable fleet snapshot; the UI just renders snapshots. Everything rebuilds from disk at launch. The only persistent state is `state.json` (preferences, alert dedup, calibration) and the quota history file. Delete both for a factory reset.
...
Claude quota and cost come from the JSON Claude Code pipes to its statusline. `cachewatch setup` wires it up. Codex support is automatic and does not modify Codex configuration.
